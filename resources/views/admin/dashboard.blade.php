...
<x-app-layout :title="$dashboard?->name ?? 'Dashboard'">
    @push('head')
        ...
        <style>
            ...
            @media (min-width:1024px) {
                ...
                .chart-card[data-w="full"] {
                    grid-column: span 6;
                }

                .chart-card[data-w="twothirds"] {
                    grid-column: span 4;
                }

                .chart-card[data-w="half"] {
                    grid-column: span 3;
                }

                .chart-card[data-w="third"] {
                    grid-column: span 2;
                }
            }
        </style>
        ...
        <div class="px-6 lg:px-8 py-16 text-center">
            ...
            <div class="flex items-center justify-center gap-2">
                ...
                <form method="POST" action="{{ route('admin.dashboards.store') }}"
                    ...
                    <button class="px-5 py-2.5 rounded-lg text-sm font-medium"
    ...
    'loopUpdateTemplate' => route('admin.loops.update', ['loop' => '__ID__']),
    'loopDestroyTemplate' => route('admin.loops.destroy', ['loop' => '__ID__']),
    ...
    'sectionDestroyTemplate' => route('admin.sections.destroy', ['section' => '__ID__']),
            ...
            <div class="flex items-center justify-between mb-4">
                <span class="text-[10.5px] font-bold uppercase tracking-[1.5px]"
                    style="color:var(--mint-deep);">Layout</span>
                <div class="flex gap-2">
            ...
            <div x-show="loops.length" x-cloak class="flex flex-wrap items-center gap-2 mb-5">
                ...
                <template x-for="lp in loops" :key="lp.id">
                    <span class="inline-flex items-center gap-2 text-[11px] px-2.5 py-1 rounded-full"
                        ...
                        <button @click="editLoop(lp)" title="Edit loop (templates apply to all)"
                            class="hover:opacity-100 opacity-60">✎</button>
                        <button @click="refreshLoop(lp)" title="Refresh — pick up new values"
                            class="hover:opacity-100 opacity-60">⟳</button>
                        <button @click="deleteLoop(lp)" title="Delete loop" class="hover:opacity-100 opacity-60"
            ...
            <template x-for="g in orderedGroups()" :key="g.key">
                <div
                    ...
                    <template x-if="g.type === 'section'">
                        <div class="flex items-center gap-3 mb-3" :class="g.level ? 'pl-5' : ''">
                            ...
                            <div class="flex items-center gap-1.5 text-xs" style="color:var(--ink-soft);">
                                <button @click="moveSection(g.section, -1)" title="Move section up"
                                    class="hover:opacity-100 opacity-60">↑</button>
                                <button @click="moveSection(g.section, 1)" title="Move section down"
                                    class="hover:opacity-100 opacity-60">↓</button>
                                <button x-show="!g.level" @click="addSubSection(g.section.id)"
                                    title="Add sub-section" class="hover:opacity-100 opacity-60 font-medium">+
                                    sub</button>
                                <button @click="renameSection(g.section)" title="Rename"
                                    class="hover:opacity-100 opacity-60">✎</button>
                                <button @click="deleteSection(g.section)" title="Delete section"
                                    class="hover:opacity-100 opacity-60" style="color:var(--coral);">🗑</button>
                            </div>
                        </div>
                    </template>
                        ...
                        <template x-for="c in chartsIn(g.sectionId)" :key="c.id">
                            <div class="chart-card rounded-xl p-5 group" :data-w="c.width || 'full'"
                                ...
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <h3 class="display text-[14.5px] font-semibold uppercase tracking-wide"
                                            x-text="c.title"></h3>
                                        <p class="text-[11.5px] mt-0.5" style="color:var(--ink-soft);"
                                            x-text="c.config.sheet + ' · grouped by ' + c.config.label_column"></p>
                                    </div>
                                    <div
                                        class="flex items-center gap-1 text-base leading-none opacity-40 group-hover:opacity-100 transition">
                                ...
                                <div :style="'height:' + (c.height ? c.height + 'px' : '16rem')"><canvas
                                        :id="'cv-' + c.id"></canvas></div>
                            </div>
                        </template>
                    </div>
                    ...
                    <template
                        x-if="g.type === 'ungrouped' && !metrics.length && !charts.length && !loading && sources.length">
            ...
            <template x-if="sources.length">
                <div class="rounded-xl p-5 mt-4" style="background:var(--panel);border:1px solid var(--line);">
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                        ...
                        <div class="flex gap-2">
                            <select x-model="tableKey" @change="loadTable()" class="rounded-lg text-xs px-3 py-2"
                        ...
                        <table class="w-full text-xs">
                            ...
                            <tbody>
                                ...
                                <template x-if="!visibleRows.length">
                                    <tr>
                                        <td :colspan="tableColumns.length || 1" class="text-center py-8 text-xs"
                                            style="color:var(--ink-soft);">No rows match.</td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
            ...
            {{-- ============================ CHART BUILDER MODAL ============================ --}}
            <div x-show="builder.open" x-cloak class="fixed inset-0 flex items-center justify-center p-4 z-50"
                style="background:rgba(14,33,29,0.55);" @click.self="closeBuilder()"
                @keydown.escape.window="builder.open && closeBuilder()">
                <div class="rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-auto"
                    style="background:var(--panel);">
                    <h3 class="display text-lg font-bold mb-5"
                        x-text="builder.captureToLoop ? 'Loop chart template' : (builder.id ? 'Configure chart' : 'New chart')">
                    </h3>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-5">
                        ...
                        <div>
                            ...
                            <select x-model="builder.type" @change="onTypeChange()"
                                class="w-full rounded-lg text-sm px-3 py-2"
                                style="border:1px solid var(--line);background:var(--panel-alt);">
                                <optgroup label="Bar">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-[10px] leading-tight" style="color:var(--ink-soft);"
                                ...
                                Only this many categories are drawn (largest first). Raise it to show every
                                status/stage.
                            </p>
                        </div>
                        ...
                        <div class="col-span-2 grid grid-cols-3 gap-4">
                            <div x-show="!builder.captureToLoop">
                                ...
                                <select x-model="builder.section_id" class="w-full rounded-lg text-sm px-3 py-2"
                                    style="border:1px solid var(--line);background:var(--panel-alt);">
                            ...
                            <div>
                                ...
                                <select x-model="builder.width" class="w-full rounded-lg text-sm px-3 py-2"
                                    style="border:1px solid var(--line);background:var(--panel-alt);">
                            ...
                            <div>
                                ...
                                <input type="number" min="160" max="900" step="20"
                                    x-model.number="builder.height" placeholder="256"
                                    class="w-full rounded-lg text-sm px-3 py-2"
                                    style="border:1px solid var(--line);background:var(--panel-alt);">
                            </div>
                        </div>
                        ...
                        <template x-if="columnsFor(builder.key).includes('Pipeline')">
                            <div class="col-span-2 grid grid-cols-2 gap-4 p-3 rounded-lg"
                                ...
                                <div>
                                    ...
                                    <select
                                        x-effect="pipelineOptions(builder.key); $nextTick(() => $el.value = filterVal(builder.filters, 'Pipeline'))"
                                ...
                                <div x-show="filterVal(builder.filters, 'Pipeline')" x-cloak>
                                    ...
                                    <select
                                        x-effect="stageOptions(builder); $nextTick(() => $el.value = filterVal(builder.filters, 'Stage'))"
                        ...
                        <div class="col-span-2 p-3 rounded-lg" style="background:var(--panel-alt);">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button type="button"
                                    ...
                                    style="border:1px solid var(--line);background:var(--panel);">+ Add
                                    condition</button>
                            </div>
                    ...
                    <div class="pt-5" style="border-top:1px solid var(--line);">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <span class="text-[10.5px] font-bold uppercase tracking-[1.5px]"
                                    style="color:var(--ink-soft);">Series (Y axis)</span>
                                <p class="text-[11px] mt-0.5" style="color:var(--ink-soft);">Each series can pull from
                                    a different integration.</p>
                            </div>
                        ...
                        <template x-for="(s, i) in builder.series" :key="i">
                            <div class="grid grid-cols-12 gap-2 items-end mb-2 p-3 rounded-lg"
                                ...
                                <div class="col-span-3">
                                    <label class="block text-[10px] mb-1"
                                        style="color:var(--ink-soft);">Source</label>
                                ...
                                <div class="col-span-3">
                                    <label class="block text-[10px] mb-1" style="color:var(--ink-soft);">Value
                                        column</label>
                                ...
                                <div class="col-span-2">
                                    <label class="block text-[10px] mb-1"
                                        style="color:var(--ink-soft);">Aggregate</label>
                    ...
                    <p x-show="builder.error" x-cloak x-text="builder.error" class="text-xs mt-4"
                        style="color:var(--coral);"></p>

                    <div class="flex items-center justify-between mt-6 gap-3">
                        <button x-show="builder.id" x-cloak @click="destroyChart()" class="text-xs font-semibold"
                            style="color:var(--coral);">Delete chart</button>
                        <div class="flex gap-2 ml-auto">
                            <button @click="closeBuilder()" class="px-4 py-2 rounded-lg text-sm font-medium"
                            ...
                            <button @click="saveChart()" :disabled="builder.saving"
                                ...
                                style="background:var(--mint-deep);">
                                <span
                                    x-text="builder.captureToLoop ? (builder.templateIndex === null ? 'Add to loop' : 'Update template') : (builder.saving ? 'Saving…' : 'Save')"></span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="metric.open" x-cloak class="fixed inset-0 flex items-center justify-center p-4 z-50"
                style="background:rgba(14,33,29,0.55);" @click.self="closeMetric()"
                @keydown.escape.window="metric.open && closeMetric()">
                <div class="rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-auto"
                    style="background:var(--panel);">
                    <h3 class="display text-lg font-bold mb-5"
                        x-text="metric.captureToLoop ? 'Loop metric template' : (metric.id ? 'Configure metric' : 'New metric')">
                    </h3>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-5">
                        ...
                        <div class="col-span-2" x-show="!metric.captureToLoop">
                            <label class="block text-xs font-semibold mb-1.5">Section</label>
                            <select x-model="metric.section_id" class="w-full rounded-lg text-sm px-3 py-2"
                                style="border:1px solid var(--line);background:var(--panel-alt);">
                                <option value="">— none (ungrouped) —</option>
                                <template x-for="opt in sectionSelectList()" :key="opt.id">
                                    <option :value="opt.id" x-text="opt.label"></option>
                    ...
                    <div class="flex gap-2 mb-5">
                        <button @click="metric.mode = 'simple'" class="px-4 py-2 rounded-lg text-xs font-semibold"
                            :style="metric.mode === 'simple' ? 'background:var(--mint-deep);color:#fff;' :
                        ...
                        <button @click="metric.mode = 'formula'; ensureVariables()"
                            ...
                            :style="metric.mode === 'formula' ? 'background:var(--mint-deep);color:#fff;' :
                                'border:1px solid var(--line);'">Formula</button>
                    </div>

                    <div x-show="metric.mode === 'simple'" class="p-4 rounded-lg mb-5"
                        style="background:var(--panel-alt);">
                    ...
                    <div x-show="metric.mode === 'formula'" x-cloak class="mb-5">
                        <div class="flex items-center justify-between mb-2">
                            <div>
                                <span class="text-[10.5px] font-bold uppercase tracking-[1.5px]"
                                    style="color:var(--ink-soft);">Variables</span>
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[11px] font-semibold"
                                            style="color:var(--ink-soft);">Name</span>
                        ...
                        <div class="mt-4">
                            ...
                            <p class="text-[11px] mt-1.5" style="color:var(--ink-soft);">
                                Only numbers and <span class="mono">+ − × ÷ ( )</span> are allowed. Division by zero
                                returns 0.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between mt-5 p-4 rounded-lg"
                        style="background:var(--panel-alt);">
                        <div>
                            <span class="text-[10.5px] uppercase tracking-wide" style="color:var(--ink-soft);">Live
                                preview</span>
                    ...
                    <p x-show="metric.error" x-cloak x-text="metric.error" class="text-xs mt-4"
                        style="color:var(--coral);"></p>

                    <div class="flex items-center justify-between mt-6 gap-3">
                        <button x-show="metric.id" x-cloak @click="destroyMetric()" class="text-xs font-semibold"
                            style="color:var(--coral);">Delete metric</button>
                        <div class="flex gap-2 ml-auto">
                            <button @click="closeMetric()" class="px-4 py-2 rounded-lg text-sm font-medium"
                                style="border:1px solid var(--line);">Cancel</button>
                            <button @click="saveMetric()" :disabled="metric.saving"
                                class="px-5 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50"
                                style="background:var(--mint-deep);">
                                <span
                                    x-text="metric.captureToLoop ? (metric.templateIndex === null ? 'Add to loop' : 'Update template') : (metric.saving ? 'Saving…' : 'Save')"></span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {{-- ============================ LOOP BUILDER MODAL ============================ --}}
            <div x-show="loop.open" x-cloak class="fixed inset-0 flex items-center justify-center p-4 z-50"
                style="background:rgba(14,33,29,0.55);" @click.self="closeLoop()"
                @keydown.escape.window="loop.open && closeLoop()">
                <div class="rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-auto"
                    style="background:var(--panel);">
                    <h3 class="display text-lg font-bold mb-1"
                        x-text="loop.id ? 'Edit loop statistics' : 'Loop statistics'"></h3>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-5 p-3 rounded-lg" style="background:var(--panel-alt);">
                        <div>
                            <label class="block text-[10px] mb-1" style="color:var(--ink-soft);">Only values where
                                (optional)</label>
                            <select x-model="loop.valueOp" class="w-full rounded text-xs px-2 py-1.5"
                                style="border:1px solid var(--line);background:var(--panel);">
                    ...
                    <div class="mb-4">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[10.5px] font-bold uppercase tracking-[1.5px]"
                                style="color:var(--ink-soft);">Metric templates</span>
                            <button @click="addMetricTemplate()" class="text-xs px-3 py-1.5 rounded-md font-medium"
                                style="border:1px solid var(--line);">+ Add metric template</button>
                        </div>
                        <template x-for="(t, i) in loop.metricTemplates" :key="i">
                            <div class="flex items-center justify-between rounded-lg px-3 py-2 mb-1.5"
                                style="background:var(--panel-alt);">
                                <span class="text-xs font-medium" x-text="t.title || 'Metric'"></span>
                                <div class="flex items-center gap-3 text-xs">
                                    <button @click="editMetricTemplate(i)"
                                        class="opacity-70 hover:opacity-100">Edit</button>
                                    <button @click="removeMetricTemplate(i)" style="color:var(--coral);">✕</button>
                                </div>
                            </div>
                        </template>
                        <p x-show="!loop.metricTemplates.length" class="text-[11px]" style="color:var(--ink-soft);">
                            None yet.</p>
                    </div>
                    ...
                    <div class="mb-5">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[10.5px] font-bold uppercase tracking-[1.5px]"
                                style="color:var(--ink-soft);">Chart templates</span>
                            <button @click="addChartTemplate()" class="text-xs px-3 py-1.5 rounded-md font-medium"
                                style="border:1px solid var(--line);">+ Add chart template</button>
                        </div>
                        <template x-for="(t, i) in loop.chartTemplates" :key="i">
                            <div class="flex items-center justify-between rounded-lg px-3 py-2 mb-1.5"
                                ...
                                <div class="flex items-center gap-3 text-xs">
                                    <button @click="editChartTemplate(i)"
                                        class="opacity-70 hover:opacity-100">Edit</button>
                                    <button @click="removeChartTemplate(i)" style="color:var(--coral);">✕</button>
                                </div>
                            </div>
                        </template>
                        <p x-show="!loop.chartTemplates.length" class="text-[11px]" style="color:var(--ink-soft);">
                            None yet.</p>
                    </div>

                    <p x-show="loop.error" x-cloak x-text="loop.error" class="text-xs mb-3"
                        style="color:var(--coral);"></p>

                    <div class="flex items-center justify-end gap-2">
                        <button @click="closeLoop()" class="px-4 py-2 rounded-lg text-sm font-medium"
                            style="border:1px solid var(--line);">Cancel</button>
...
function dashboard(cfg) {
    return {
        ...
        // Swap the __ID__ placeholder in a server-generated URL template.
        // Every update/destroy/refresh endpoint is built this way now, so the
        // JS never has to guess the app's base path (docroot, subfolder, etc.)
        // — route() on the Blade side already resolved it correctly.
        withId(template, id) {
        ...
        async saveLoop() {
            ...
            const url = editing
                ? this.withId(this.cfg.loopUpdateTemplate, this.loop.id)
                : this.cfg.loopsStore;
        ...
        async refreshLoop(loop) {
            await fetch(this.withId(this.cfg.loopRefreshTemplate, loop.id), {
                method: 'POST',
                headers: { Accept: 'application/json', 'X-CSRF-TOKEN': this.csrf },
            });
            await Promise.all([this.loadLoops(), this.loadSections(), this.loadMetrics(), this.loadCharts()]);
        ...
        async deleteLoop(loop) {
            ...
            await fetch(this.withId(this.cfg.loopDestroyTemplate, loop.id), {
                method: 'DELETE',
                headers: { Accept: 'application/json', 'X-CSRF-TOKEN': this.csrf },
            });
            await Promise.all([this.loadLoops(), this.loadSections(), this.loadMetrics(), this.loadCharts()]);
        ...
        async renameSection(section) {
            ...
            const res = await fetch(this.withId(this.cfg.sectionUpdateTemplate, section.id), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-CSRF-TOKEN': this.csrf },
        ...
        async deleteSection(section) {
            ...
            const res = await fetch(this.withId(this.cfg.sectionDestroyTemplate, section.id), {
                method: 'DELETE',
                headers: { Accept: 'application/json', 'X-CSRF-TOKEN': this.csrf },
            });
            if (res.ok) {
        ...
        async saveChart() {
            ...
            const url = editing
                ? this.withId(this.cfg.chartUpdateTemplate, this.builder.id)
                : this.cfg.chartsStore;
        ...
        async destroyChart() {
            ...
            await fetch(this.withId(this.cfg.chartDestroyTemplate, this.builder.id), {
                method: 'DELETE',
        ...
        async saveMetric() {
            ...
            const url = editing
                ? this.withId(this.cfg.metricUpdateTemplate, this.metric.id)
                : this.cfg.metricsStore;
        ...
        async destroyMetric() {
            ...
            await fetch(this.withId(this.cfg.metricDestroyTemplate, this.metric.id), {
                method: 'DELETE',
                headers: { 'X-CSRF-TOKEN': this.csrf },
            });
            this.metric.open = false;
