...
<body class="min-h-screen">
    ...
    <div x-data="{ mobileNav: false }" class="lg:grid lg:grid-cols-[248px_1fr] min-h-screen">
        ...
        {{-- MOBILE TOPBAR --}}
        <div class="lg:hidden flex items-center justify-between px-4 py-3" style="background:var(--sidebar);">
        ...
        {{-- SIDEBAR --}}
        <aside :class="mobileNav ? 'block' : 'hidden'" class="lg:block px-5 py-7 flex-col"
            ...
            <nav class="space-y-1">
                ...
                @foreach ($menuTree ?? [] as $item)
                    @php
                        $active =
                            $item->dashboard &&
                            request()->routeIs('admin.dashboards.show') &&
                            request()->route('dashboard')?->slug === $item->dashboard->slug;
                        $childActive = $item->children->contains(
                            fn($c) => $c->dashboard &&
                                request()->routeIs('admin.dashboards.show') &&
                                request()->route('dashboard')?->slug === $c->dashboard->slug,
                        );
                    @endphp
                    ...
                    @else
                        ...
                        <div x-data="{ open: {{ $childActive ? 'true' : 'false' }} }">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-150"
                                ...
                                x-transition:leave-end="opacity-0 -translate-y-1" class="ms-3 ps-3 mt-0.5 space-y-1"
                                ...
                                @foreach ($item->children as $child)
                                    @php
                                        $childIsActive =
                                            $child->dashboard &&
                                            request()->routeIs('admin.dashboards.show') &&
                                            request()->route('dashboard')?->slug === $child->dashboard->slug;
                                    @endphp
                ...
                <div class="mt-4 pt-4 space-y-1" style="border-top:1px solid var(--sidebar-line);">
                    @foreach ($tools as $link)
                        ...
                        <a href="{{ route($link['route']) }}" @click="mobileNav = false"
                            ...
                            style="{{ $active ? 'background:rgba(43,227,143,0.14);color:var(--mint);font-weight:600;box-shadow:inset 3px 0 0 0 var(--mint);' : 'color:#B9CCC4;' }}">
                            <span
                                class="text-base leading-none">{{ $link['icon'] }}</span><span>{{ $link['label'] }}</span>
                        </a>
                    @endforeach
                </div>
            </nav>
            ...
            <div class="mt-8 pt-5" style="border-top:1px solid var(--sidebar-line);">
                ...
                @foreach ($integrations as $integration)
                    <div class="mono text-[10px] mt-2" style="color:#4A6459;">
                        {{ $integration->name }} ·
                        {{ $integration->last_synced_at?->diffForHumans(short: true) ?? 'never synced' }}
                    </div>
                @endforeach
            </div>
