...
/**
 * Metric engine — UNCHANGED except the constructor now takes SourceManager and
 * reads rows from the active source. Everything else (simple aggregates, the
 * safe shunting-yard formula evaluator) is byte-for-byte what it was, so a
 * formula like {won} / {messaged} * 100 works identically against GHL rows.
 */
class MetricService
{
    ...
    private function evaluateFormula(Metric $metric): float
    {
        ...
        $resolved = preg_replace_callback('/\{(\w+)\}/', function ($m) use ($variables) {
            $name = $m[1];
            ...
            return (string) $this->computeSimple($variables[$name]);
        }, $expression);
